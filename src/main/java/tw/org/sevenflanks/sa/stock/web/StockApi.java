...
public class StockApi {
	...
	@GetMapping(value = "/check/year/{year}", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
	public Flux<DataStoringModel> check(@PathVariable @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) Year year) {
	...
	@GetMapping(value = "/init/{type}/month/{yearMonth}", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
	public Flux<DataStoringModel> init(@PathVariable String type, @PathVariable @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) YearMonth yearMonth) {
		final Iterator<LocalDate> dates = toDateIterator(
